...
df = pd.read_csv("Santiago_Diaz_Pabon_Plot_and_Store_Rep_1.2.csv")
triceps = df ["R TRICEPS BRACHII: EMG 6"]
biceps = df['R BICEPS BRACHII: EMG 5']
fs = 2000
plt.subplot(2,1,1)
plt.plot(triceps)
plt.title('triceps ej 1')
plt.subplot(2,1,2)
plt.plot(biceps)
plt.title('biceps ej 1')
plt.show()
#rectificado
...
plt.title('tricep(rojo) y bicep(magenta) en el primer ejercicio')
plt.show()
#rms del tricep
wn = 3/(fs/2)
...
plt.plot(sig,'r')
plt.plot(sig1,'m')
plt.title('rms de los musculos en el ejercicio 1')
plt.show()
